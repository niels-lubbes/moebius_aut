...
P^1xP^1 into projective 8-space P^8. This example shows that there is a hyperquadric Q in P^8 
of signature (4,5) such that Aut(P^1xP^1) is contained in Aut(Q).
...
However, since --- for our purposes --- we consider the 
matrices up to scalar multiplication we have that 
the Lie subalgebra so2+so2 is generated by
...
For example we obtain a parametrization of a ring cyclide if
we compose the parametrization of the double Segre surface with the linear projection 
...
__Example 3: Computing G-invariant quadratic forms in the ideal of a projection of the double Segre surface__
